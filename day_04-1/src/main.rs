	...
	for drawned in nb_drawned {
		...
		match check_winner(&boards) {
			Some(winner) => {
				...
				println!("{:#?}", winner);
				println!("Score: {}", calculate_score(winner, drawned));
				break;
			},
			None => continue,
		}
...
fn check_winner(boards: &Vec<Board>) -> Option<&Board> {
	for board in boards {
		if let Some(winner) = check_rows(&board) {
			return Some(winner);
		}
		else if let Some(winner) = check_columns(board) {
			return Some(winner);
		}
	}
	None
