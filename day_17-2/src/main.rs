...
fn main() {
	...
	let x_range = Range {
		min: calc_min_x(target_area.x.min),
		max: target_area.x.max,
	};
	let y_range = Range {
		min: target_area.y.min,
		max: target_area.y.min.abs() - 1,
	};
	let mut valid_solutions: Vec<Point> = Vec::new();

	for x in x_range.min..x_range.max + 1 {
		for y in y_range.min..y_range.max + 1 {
			let steps = shoot(&target_area, Point {x, y});
			if step_in_target(&target_area, steps.last().unwrap()) {
				// print_map(&target_area, &steps);
				valid_solutions.push(Point {x, y});
			}
		}
	}
	println!("Valid shots: {:?}", valid_solutions);
	println!("nb valid solutions: {}", valid_solutions.len());
}

fn calc_min_x(area_x_min: i32) -> i32 {
	let mut min_x = 0;

	for i in 0..area_x_min + 1 {
		min_x += i;
		if min_x >= area_x_min {
			return i;
		}
...
fn step_in_target(target_area: &Area, step: &Point) -> bool {
	!(step.x < target_area.x.min || step.x > target_area.x.max
		|| step.y < target_area.y.min || step.y > target_area.y.max)
}
